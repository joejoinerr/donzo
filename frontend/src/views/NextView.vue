<template>
    <h1 class="text-2xl mb-8 font-bold text-gray-600">Next ({{ incompleteActionsCount }})</h1>
...
const { nextActions } = storeToRefs(actionStore);
const incompleteActions = computed(() => nextActions.value?.filter(action => !action.completed) || []);
const completeActions = computed(() => nextActions.value?.filter(action => action.completed) || []);
const incompleteActionsCount = computed(() => incompleteActions.value?.length || 0);
const viewEmpty = computed(() => (nextActions.value?.length || 0) === 0);
</script>
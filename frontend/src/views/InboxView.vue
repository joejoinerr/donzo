<template>
    <h1 class="text-2xl mb-8 font-bold text-gray-600">Inbox ({{ incompleteActionsCount }})</h1>
...
const incompleteActions = computed(() => inboxActions.value?.filter(action => !action.completed) || []);
const completeActions = computed(() => inboxActions.value?.filter(action => action.completed) || []);

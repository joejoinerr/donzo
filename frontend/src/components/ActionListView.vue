...
const incompleteActions = computed(() => props.actions?.filter(action => !action.completed) || []);
const completeActions = computed(() => props.actions?.filter(action => action.completed) || []);
const incompleteActionsCount = computed(() => incompleteActions.value?.length || 0);
const viewEmpty = computed(() => (props.actions?.length || 0) === 0);
</script>
